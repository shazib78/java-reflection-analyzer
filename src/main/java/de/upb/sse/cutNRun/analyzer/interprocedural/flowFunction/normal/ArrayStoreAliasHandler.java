...
public class ArrayStoreAliasHandler implements AliasHandler {
    ...
    public ArrayStoreAliasHandler(SootMethod method, Stmt curr, Value lhs,
            ...
            for (AccessPath alias : aliases) {
                ...
                if (base instanceof JimpleUpVal) {
                    ...
                    if(!delegate.equals(arrayRef.getBase())){
                        JArrayRef newRef = new JArrayRef((Local) delegate, arrayRef.getIndex());
                        res.add(new DFF(newRef, curr, view));
                    }
                }
            }
        }
